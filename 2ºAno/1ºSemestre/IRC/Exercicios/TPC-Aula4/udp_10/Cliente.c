...
{
	SOCKET sockfd;
	int msg_len, iResult, resposta;
	struct sockaddr_in serv_addr, local_addr, check;
	WSADATA wsaData;
	int nbytes;
	...
	nbytes = recvfrom(sockfd, (char*)&resposta, sizeof(buffer), 0, (struct sockaddr*)&check, &check_len);
	...
	if (check.sin_port == serv_addr.sin_port && strcmp(inet_ntoa(check.sin_addr), inet_ntoa(serv_addr.sin_addr)) == 0) {
		printf("\nMensagem {%d} recebida do servidor!\n",resposta);
	}
	else {
		printf("\nMensagem {%d} recebida de um impostor com IP:{%s} e Port :{%s}.\n",resposta,inet_ntoa(check.sin_addr),ntohs(check.sin_port));
	}
	/*========================= FECHA O SOCKET ===========================*/
